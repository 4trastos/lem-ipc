...
int main(int argc, char **argv)
{
    ...
    prev_game_board = (int *)malloc(sizeof(int) * board_size);
    if (!prev_game_board) {
        ft_printf("❌ Error: Failed to allocate memory ❌\n");
        shmdt(shm_prt);
        return (1);
    }
    ft_memcpy(prev_game_board, game_board, board_size * sizeof(int));

    draw_static_header(board_dim, players, teams);

    while (teams > 1 && inactive_count < 3)
    {
        players = *(int *)(shm_prt + sizeof(int));
        teams = *(int *)(shm_prt + 2 * sizeof(int));

        if (ft_memcmp(game_board, prev_game_board, board_size * sizeof(int)) == 0)
            inactive_count++;
        else
            inactive_count = 0;
